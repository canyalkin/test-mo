...
public class PageRankChooserNew implements ClusterChooseSentenceStrategy  {

	private static final Logger LOGGER = Logger.getLogger(PageRankChooserNew.class);
	...
	public List<Integer> createSentence(List<Cluster> clusterList,
		...
		for(int indx=0;indx<clusterList.size();indx++){
			int sentenceIndex=indexesFromCluster.get(indx).get(0);
	...
	private class RankIndex implements Comparable<RankIndex>{
		...
		private PageRankChooserNew getOuterType() {
			return PageRankChooserNew.this;
		}
